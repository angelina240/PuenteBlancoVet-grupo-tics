...
public class ReportServiceImpl implements ReportService {
        ...
        public String obtenerNombreVeterinarioPorCorreo(String correo) {
                User user = userRepository.findByCorreo(correo)
                                .orElseThrow(() -> new RuntimeException(
                                                "Veterinario no encontrado con correo: " + correo));
                return user.getNombres() + " " + user.getApellidoPaterno();
        }

        // --- Citas reprogramadas ---
        @Override
        public void exportCitasReprogramadasReport(OutputStream outputStream, String startDate, String endDate,
                        String emitidoPor) {
                ...
                List<CitaReprogramadaDTO> datos = citas.stream()
                                .filter(c -> c.getCantidadReprogramaciones() != null
                                                && c.getCantidadReprogramaciones() > 0)
                                .map(cita -> new CitaReprogramadaDTO(
                                                cita.getId(),
                                                cita.getUsuario().getNombres(),
                                                cita.getPet().getName(),
                                                cita.getServicio().getDescripcion(),
                ...
                pdfReportService.generarPdfCitasReprogramadas(start, end, outputStream, datos, emitidoPor);
        }
        ...
        public void generarPdfCitasPorMascota(LocalDate startDate, LocalDate endDate, OutputStream outputStream,
                ...
                List<CitaPorMascotaDTO> datos = obtenerCitasPorMascotaFiltrado(startDate, endDate, tipoMascota,
                                cliente);
        ...
        public void generarPdfCitasPorFecha(LocalDate startDate, LocalDate endDate, OutputStream outputStream,
                ...
                pdfReportService.generarPdfCitasPorFecha(startDate, endDate, outputStream, datos, emitidoPor,
                                tipoServicio);
        }
